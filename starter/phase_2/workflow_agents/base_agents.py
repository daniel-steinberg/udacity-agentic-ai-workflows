...
class RoutingAgent():
    ...
    def route_to_agent_with_context(self, user_input, context):
        input_emb = self.get_embedding(user_input)
        best_agent = None
        best_score = -1
        ...
        print(f"[Router] Best agent: {best_agent['name']} (score={best_score:.3f})")
        enriched_prompt = f"{user_input}. Additional context from previous steps: {context}"
        return best_agent["func"](enriched_prompt)
