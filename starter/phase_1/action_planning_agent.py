# Complete: 1 - Import all required libraries, including the ActionPlanningAgent
from workflow_agents.base_agents import ActionPlanningAgent
import os
from dotenv import load_dotenv

# Complete: 2 - Load environment variables and define the openai_api_key variable with your OpenAI API key
load_dotenv()
...
# Complete: 3 - Instantiate the ActionPlanningAgent, passing the openai_api_key and the knowledge variable
planning_agent = ActionPlanningAgent(open_ai_key, knowledge)

# Complete: 4 - Print the agent's response to the following prompt: "One morning I wanted to have scrambled eggs"
prompt = "One morning I wanted to have scrambled eggs"
response = planning_agent.extract_steps_from_prompt(prompt)
print(response)
